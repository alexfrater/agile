...
class ToyModel(torch.nn.Module):
    def __init__(self, in_channels=32, out_channels=32, layer_count=1, hidden_dimension=32, precision = torch.float32):
        ...
        # self.linear3 = MLP_Model(in_channels, out_channels) 
        # self.linear3.name  = 'linear_embedder3'
        # self.layers.append(self.linear3) #Used to map weights in SDK

        # self.linear4 = MLP_Model(in_channels, out_channels) 
        # self.linear4.name  = 'linear_embedder4'
        # self.layers.append(self.linear4) #Used to map weights in SDK
    ...
    def forward(self, edge_index,x,edge_attr):
        ...
        # outputs_sub_model2,edge_attr = self.linear2(edge_attr)

        # outputs_sub_model3,x = self.linear3(x)

        # outputs_sub_model4,y = self.linear4(x)
...
inputs = [dataset.edge_index,dataset.x,dataset.edge_attr]
out = model(*inputs)
